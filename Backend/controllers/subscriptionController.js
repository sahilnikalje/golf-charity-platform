...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    const charityContrib =
      ((planConfig.amount / 100) * (user.charityPercentage || 50)) / 100;
...
const handleSubscriptionEvent = async (stripeSubscription, status) => {
  try {
    ...
    const charityContrib = metadata.charityContribution
      ? parseFloat(metadata.charityContribution)
      : 0;
    const prizeContrib = metadata.prizePoolContribution
      ? parseFloat(metadata.prizePoolContribution)
      : 0;
